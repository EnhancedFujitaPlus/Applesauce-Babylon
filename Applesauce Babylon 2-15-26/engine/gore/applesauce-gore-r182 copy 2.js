...
export class ApplesauceGore {
    ...
    update(engine) {
        ...
        for (let i = this.blood.length - 1; i >= 0; i--) {
            ...
            // ===================================
            // SMOOTHED TERRAIN-AWARE BLOOD PHYSICS
            // Frame throttling to prevent flickering
            // ===================================
            if (particle.velocity) {
                ...
                if (particle.terrainFrameCount === undefined) {
                    particle.terrainFrameCount = Math.floor(Math.random() * 5); // Stagger updates
                    particle.cachedGroundLevel = 0;
                    particle.cachedSlopeX = 0;
                    particle.cachedSlopeZ = 0;
                }
                
                particle.terrainFrameCount++;
                
                // ===================================
                // FRAME THROTTLING: Only sample terrain every 5 frames per particle
                // This MASSIVELY reduces flickering and improves performance
                // ===================================
                const shouldUpdateTerrain = particle.terrainFrameCount % 5 === 0;
                
                // Multi-point terrain sampling (only when needed)
                const terrainModule = engine.modules.terrain;
                if (terrainModule) {
                    const px = particle.position.x;
                    const pz = particle.position.z;
                    
                    let groundLevel = particle.cachedGroundLevel || 0;
                    let slopeX = particle.cachedSlopeX || 0;
                    let slopeZ = particle.cachedSlopeZ || 0;
                    ...
                    if (shouldUpdateTerrain) {
                        ...
                        groundLevel = Math.max(hCenter, hFront, hBack, hLeft, hRight);
                        
                        // Calculate terrain slope (cache these values)
                        slopeX = (hRight - hLeft) / (sampleRadius * 2);
                        slopeZ = (hFront - hBack) / (sampleRadius * 2);
                        
                        // Cache for reuse
                        particle.cachedGroundLevel = groundLevel;
                        particle.cachedSlopeX = slopeX;
                        particle.cachedSlopeZ = slopeZ;
                    }
                    
                    // ===================================
                    // TERRAIN SLOPE FLOW (BLOOD RUNS DOWNHILL)
                    // Uses cached slope values for smooth flow
                    // ===================================
                    if (particle.position.y <= groundLevel + 0.15) {
                        // Blood flows downhill using cached slope
                        const flowStrength = 0.006; // Slightly reduced for smoother flow
                        particle.velocity.x -= slopeX * flowStrength;
                        particle.velocity.z -= slopeZ * flowStrength;
                        
                        // Apply friction when on ground
                        particle.velocity.multiplyScalar(0.94); // Slightly increased friction
                    }
