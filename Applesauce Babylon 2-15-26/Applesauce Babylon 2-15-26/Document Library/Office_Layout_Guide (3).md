...
**"Textures are flickering (z-fighting)!"**
- This happens when two surfaces are at the exact same position
- Fix: Offset surfaces by 0.02-0.05 units
- Example: If floor is at y=0.05, put decals at y=0.10
- Ground rule: Base ground at 0.05, decorations at 0.10+, tall objects at 0.15+

---

## PREVENTING Z-FIGHTING (FLICKERING TEXTURES)

Z-fighting occurs when two surfaces occupy the same space. Always offset overlapping objects:

```javascript
// BAD - Both at same height
const floor = new THREE.Mesh(...);
floor.position.y = 0;
const decal = new THREE.Mesh(...);
decal.position.y = 0; // WILL FLICKER!

// GOOD - Offset by small amount
const floor = new THREE.Mesh(...);
floor.position.y = 0.05;
const decal = new THREE.Mesh(...);
decal.position.y = 0.10; // 0.05 higher - no flickering!
```

**Recommended Y-position layers:**
- **Base ground**: 0.05
- **Ground decals** (lines, markings): 0.10-0.15
- **Raised surfaces** (grass islands): 0.20-0.25
- **Objects on ground** (desks, cars): Start at 0.5+
