...
window.Level41Config = {
    ...
    onLevelStart: async function(game) {
        ...
        // Initialize gore (if not already loaded by engine)
        if (!game.gore) {
            console.log('⚠️ Warning: Gore system not found - level may not work properly');
            console.log('💡 Make sure BabylonGorePhysics is loaded before level');
        } else {
            // Configure gore for this level
            game.gore.config.damageThreshold = 5;
            game.gore.config.severThreshold = 10;
            game.gore.config.explodeThreshold = 25;
