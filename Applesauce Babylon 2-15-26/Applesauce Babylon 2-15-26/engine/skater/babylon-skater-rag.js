/**
 * APPLESAUCE Skater Module - ARCADE PHYSICS VERSION
 * 
 * Uses velocity-based movement for fast, responsive arcade feel
 * Matches the original Three.js movement system
 */

export class BabylonSkater {
    constructor(scene, debug = false) {
        this.scene = scene;
        this.skaterRoot = null;
        this.deck = null;
        this.body = null;
        this.head = null;
        this.physicsAggregate = null;
        this.physicsCollider = null;
        this.debug = debug;
        this.rayHelper = null;
        
        // Store ALL part references for ragdoll
        this.arms = [];
        this.legs = [];
        this.wheels = [];
        this.allParts = [];         // Every mesh that becomes a ragdoll piece
        this.partLocalPositions = new Map();  // Original local positions for reset
        this.partLocalRotations = new Map();
        
        // RAGDOLL STATE
        this.ragdollActive = false;
        this.ragdollAggregates = [];  // Physics bodies to dispose on reset
        
        // ARCADE MOVEMENT STATE
        this.state = {
            speed: 0,
            rotation: 0,
            acceleration: 0.015,
            maxSpeed: 0.8,
            friction: 0.97,
            turnSpeed: 0.04,
            gravity: -0.015,
            jumpVelocity: 0,
            jumping: false,
            grounded: false,
            grinding: false,
            currentRail: null,
            spinning: false,
            spinRotation: 0,
            currentTrick: '',
            landedClean: true,
            bailed: false,
            trickChain: [],  // Track all tricks in current combo
            trickCount: 0,   // Number of tricks this jump
            stunned: false,  // Can't jump when stunned
            stunnedTimer: 0  // Frames remaining stunned
        };
        
        console.log('🛹 Babylon Skater ARCADE module loaded');
    }
    
    spawn(config = {}) {
        const x = config.x || 0;
        const y = config.y || 5;
        const z = config.z || 0;
        const deckColor = config.deckColor || new BABYLON.Color3(1, 0.08, 0.58);
        const bodyColor = config.bodyColor || new BABYLON.Color3(0.2, 0.2, 0.2);
        const skinColor = config.skinColor || new BABYLON.Color3(1, 0.86, 0.67);
        
        // Create root transform node
        const skaterRoot = new BABYLON.TransformNode("skaterRoot", this.scene);
        skaterRoot.position = new BABYLON.Vector3(x, y, z);
        
        // SKATEBOARD DECK
        const deck = BABYLON.MeshBuilder.CreateBox(
            "deck",
            { width: 0.8, height: 0.1, depth: 2.5 },
            this.scene
        );
        deck.position = new BABYLON.Vector3(0, 0.3, 0);
        deck.parent = skaterRoot;
        
        const deckMat = new BABYLON.StandardMaterial("deckMat", this.scene);
        deckMat.diffuseColor = deckColor;
        deckMat.specularColor = new BABYLON.Color3(0.3, 0.3, 0.3);
        deck.material = deckMat;
        deck.checkCollisions = false;
        
        this.deck = deck;
        
        // WHEELS
        const wheelPositions = [
            { x: -0.3, y: 0.15, z: -0.8 },
            { x: 0.3, y: 0.15, z: -0.8 },
            { x: -0.3, y: 0.15, z: 0.8 },
            { x: 0.3, y: 0.15, z: 0.8 }
        ];
        
        const wheelMat = new BABYLON.StandardMaterial("wheelMat", this.scene);
        wheelMat.diffuseColor = new BABYLON.Color3(0, 0, 0);
        
        this.wheels = [];
        wheelPositions.forEach((pos, i) => {
            const wheel = BABYLON.MeshBuilder.CreateCylinder(
                `wheel${i}`,
                { height: 0.1, diameter: 0.3, tessellation: 12 },
                this.scene
            );
            wheel.position = new BABYLON.Vector3(pos.x, pos.y, pos.z);
            wheel.rotation.z = Math.PI / 2;
            wheel.parent = skaterRoot;
            wheel.material = wheelMat;
            wheel.checkCollisions = false;
            this.wheels.push(wheel);
        });
        
        // BODY
        const body = BABYLON.MeshBuilder.CreateBox(
            "body",
            { width: 0.6, height: 1.2, depth: 0.4 },
            this.scene
        );
        body.position = new BABYLON.Vector3(0, 1.2, -0.2);
        body.parent = skaterRoot;
        
        const bodyMat = new BABYLON.StandardMaterial("bodyMat", this.scene);
        bodyMat.diffuseColor = bodyColor;
        body.material = bodyMat;
        body.checkCollisions = false;
        
        this.body = body;
        
        // HEAD
        const head = BABYLON.MeshBuilder.CreateSphere(
            "head",
            { diameter: 0.6, segments: 8 },
            this.scene
        );
        head.position = new BABYLON.Vector3(0, 2.1, -0.2);
        head.parent = skaterRoot;
        
        const headMat = new BABYLON.StandardMaterial("headMat", this.scene);
        headMat.diffuseColor = skinColor;
        head.material = headMat;
        head.checkCollisions = false;
        
        this.head = head;
        
        // ARMS
        this.arms = [];
        [-1, 1].forEach(side => {
            const arm = BABYLON.MeshBuilder.CreateBox(
                `arm${side}`,
                { width: 0.2, height: 0.8, depth: 0.2 },
                this.scene
            );
            arm.position = new BABYLON.Vector3(side * 0.4, 1.2, -0.2);
            arm.parent = skaterRoot;
            arm.material = bodyMat;
            arm.checkCollisions = false;
            this.arms.push(arm);
        });
        
        // LEGS
        this.legs = [];
        [-1, 1].forEach(side => {
            const leg = BABYLON.MeshBuilder.CreateBox(
                `leg${side}`,
                { width: 0.2, height: 0.6, depth: 0.2 },
                this.scene
            );
            leg.position = new BABYLON.Vector3(side * 0.2, 0.6, -0.2);
            leg.parent = skaterRoot;
            leg.material = bodyMat;
            leg.checkCollisions = false;
            this.legs.push(leg);
        });
        
        // REGISTER ALL PARTS for ragdoll system
        this.allParts = [this.deck, this.body, this.head, ...this.arms, ...this.legs, ...this.wheels];
        for (const part of this.allParts) {
            this.partLocalPositions.set(part, part.position.clone());
            this.partLocalRotations.set(part, part.rotation.clone());
        }
        
        // PHYSICS - Simple capsule for collision only
        const physicsCollider = BABYLON.MeshBuilder.CreateCapsule(
            "skaterCollider",
            { 
                height: 1.5,
                radius: 0.6
            },
            this.scene
        );
        physicsCollider.position = new BABYLON.Vector3(x, y, z);
        physicsCollider.isVisible = false;
        
        // Add physics with kinematic motion (we control movement manually)
        const aggregate = new BABYLON.PhysicsAggregate(
            physicsCollider,
            BABYLON.PhysicsShapeType.CAPSULE,
            { 
                mass: 1,
                restitution: 0,
                friction: 0
            },
            this.scene
        );
        
        // Use ANIMATED motion type - we move it manually, but it still collides
        aggregate.body.setMotionType(BABYLON.PhysicsMotionType.ANIMATED);
        aggregate.body.disablePreStep = false;
        
        this.physicsAggregate = aggregate;
        this.physicsCollider = physicsCollider;
        this.skaterRoot = skaterRoot;
        
        // Initialize rotation
        this.state.rotation = 0;
        
        console.log(`🛹 Skater spawned at (${x}, ${y}, ${z}) - ARCADE MODE`);
        
        return {
            root: skaterRoot,
            collider: physicsCollider,
            aggregate: aggregate,
            parts: {
                deck: this.deck,
                body: this.body,
                head: this.head
            }
        };
    }
    
    update() {
        if (!this.skaterRoot || !this.physicsCollider) return;
        
        // Skip arcade movement when ragdolled — physics engine handles it
        if (this.ragdollActive) return;
        
        // Update stunned timer
        if (this.state.stunned) {
            this.state.stunnedTimer--;
            if (this.state.stunnedTimer <= 0) {
                this.state.stunned = false;
                this.state.stunnedTimer = 0;
                console.log('💚 Recovered from bail - can jump again!');
            }
        }
        
        // Apply friction
        this.state.speed *= this.state.friction;
        
        // Calculate forward direction
        const forward = new BABYLON.Vector3(
            Math.sin(this.state.rotation),
            0,
            Math.cos(this.state.rotation)
        );
        
        // Update position based on speed
        this.physicsCollider.position.x += forward.x * this.state.speed;
        this.physicsCollider.position.z += forward.z * this.state.speed;
        
        // Update rotation
        this.physicsCollider.rotation.y = this.state.rotation;
        
        // GROUND DETECTION with raycast
        const rayStart = this.physicsCollider.position.clone();
        const rayDirection = new BABYLON.Vector3(0, -1, 0);
        const rayLength = 5;
        
        const ray = new BABYLON.Ray(rayStart, rayDirection, rayLength);
        const hit = this.scene.pickWithRay(ray, (mesh) => {
            return mesh !== this.physicsCollider && 
                   mesh !== this.skaterRoot && 
                   mesh.name !== 'skaterCollider' &&
                   !mesh.name.includes('wheel') &&
                   !mesh.name.includes('deck') &&
                   !mesh.name.includes('body') &&
                   !mesh.name.includes('head') &&
                   !mesh.name.includes('arm') &&
                   !mesh.name.includes('leg') &&
                   !mesh.name.includes('helmet');
        });
        
        // Debug ray
        if (this.debug) {
            if (this.rayHelper) {
                this.rayHelper.dispose();
            }
            this.rayHelper = new BABYLON.RayHelper(ray);
            this.rayHelper.show(this.scene, new BABYLON.Color3(0, 1, 1));
        }
        
        // JUMP AND GRAVITY
        if (this.state.jumping || this.state.grinding) {
            if (!this.state.grinding) {
                // Apply gravity
                this.physicsCollider.position.y += this.state.jumpVelocity;
                this.state.jumpVelocity += this.state.gravity;
            }
            
            // Check if landed
            if (hit && hit.hit) {
                const groundLevel = hit.pickedPoint.y + 1.0; // Offset for skater height
                
                // Only land if we're descending (negative velocity) and close to ground
                const closeToGround = this.physicsCollider.position.y <= (groundLevel + 0.1);
                const descending = this.state.jumpVelocity < -0.05;
                
                if (closeToGround && descending && !this.state.grinding) {
                    this.physicsCollider.position.y = groundLevel;
                    this.state.jumping = false;
                    this.state.jumpVelocity = 0;
                    
                    console.log('📍 LANDED!');
                    
                    // BAIL DETECTION - Check deck rotation on landing
                    if (this.state.spinning && this.state.trickCount > 0) {
                        // Calculate how close to a full rotation we are
                        const rotationMod = Math.abs(this.deck.rotation.x) % (Math.PI * 2);
                        const cleanLanding = rotationMod < 0.5 || rotationMod > (Math.PI * 2 - 0.5);
                        
                        if (!cleanLanding) {
                            // BAILED! Deck not level
                            this.state.bailed = true;
                            this.state.landedClean = false;
                            this.state.currentTrick = 'BAILED!';
                            
                            // BAIL PUNISHMENT - Force grounded and stun
                            this.state.grounded = true;  // Stay down!
                            this.state.jumping = false;
                            this.state.jumpVelocity = 0;
                            this.state.speed *= 1.0;  // Kill most speed 0.3
                            this.state.stunned = true;  // Can't jump
                            this.state.stunnedTimer = 60;  // 1 second stun
                            
                            console.log('💀 BAIL! Speed killed, stunned for 1 sec');
                        } else {
                            // Clean landing!
                            this.state.landedClean = true;
                            this.state.bailed = false;
                            this.state.grounded = true;
                            console.log('✅ Clean landing!');
                        }
                    } else {
                        // Normal landing (no tricks)
                        this.state.grounded = true;
                    }
                    
                    // Reset trick state after a delay (let game loop handle scoring)
                    this.state.spinning = false;
                    this.state.spinRotation = 0;
                    this.deck.rotation.x = 0;
                    
                    // Don't clear trickChain here - let the game loop score it first!
                }
            }
        } else {
            // On ground, match terrain
            if (hit && hit.hit) {
                const groundLevel = hit.pickedPoint.y + 1.0;
                this.physicsCollider.position.y = groundLevel;
                this.state.grounded = true;
            } else {
                // No ground detected, start falling
                this.state.grounded = false;
                this.state.jumping = true;
                this.state.jumpVelocity = 0;
            }
        }
        
        // TRICK ANIMATION
        if (this.state.spinning) {
            this.state.spinRotation += 0.3;
            this.deck.rotation.x = this.state.spinRotation;
        }
        
        // Sync visual model to physics position
        this.skaterRoot.position = this.physicsCollider.position.clone();
        this.skaterRoot.position.y -= 1.0; // Offset for visual
        this.skaterRoot.rotation.y = this.state.rotation;
    }
    
    // ARCADE MOVEMENT METHODS
    accelerateForward() {
        this.state.speed = Math.min(this.state.speed + this.state.acceleration, this.state.maxSpeed);
    }
    
    accelerateBackward() {
        this.state.speed = Math.max(this.state.speed - this.state.acceleration, -this.state.maxSpeed * 0.5);
    }
    
    
    accelerate() {
        if (Math.abs(this.state.speed) < this.state.maxSpeed) {
            this.state.speed += this.state.acceleration;
        }
    }
    
    brake() {
        if (this.state.speed > 0) {
            this.state.speed -= this.state.acceleration * 2; // Brake faster than accel
        } else if (this.state.speed < 0) {
            this.state.speed += this.state.acceleration * 2;
        }
    }
    
    turnLeft() {
        this.state.rotation += this.state.turnSpeed;
    }
    
    turnRight() {
        this.state.rotation -= this.state.turnSpeed;
    }
    
    jump() {
        // Can't jump if stunned from bail!
        if (this.state.stunned) {
            console.log('❌ Cannot jump - STUNNED from bail!');
            return;
        }
        
        if (this.state.grounded && !this.state.jumping) {
            this.state.jumping = true;
            this.state.jumpVelocity = 0.35;
            this.state.grounded = false;
            this.state.trickChain = [];  // Reset trick chain
            this.state.trickCount = 0;
            this.state.spinning = false;
            
            // Give minimum forward momentum if standing still
            if (Math.abs(this.state.speed) < 0.05) {
                this.state.speed = 0.2; // Small forward push
                console.log('🛹 JUMPED from standstill - adding forward momentum');
            } else {
                console.log('🛹 JUMPED with speed:', this.state.speed.toFixed(3));
            }
            
            console.log('  Position:', this.physicsCollider.position.y.toFixed(2));
        } else {
            console.log('❌ Cannot jump - grounded:', this.state.grounded, 'jumping:', this.state.jumping);
        }
    }
    
    doTrick(trickType = 'kickflip') {
        console.log('🎯 Trick attempt:', trickType, 'Grounded:', this.state.grounded);
        
        // Allow tricks anytime you're airborne!
        if (!this.state.grounded) {
            this.state.spinning = true;
            this.state.currentTrick = trickType;
            this.state.bailed = false;
            this.state.landedClean = false;
            
            // Add to trick chain
            this.state.trickChain.push(trickType);
            this.state.trickCount++;
            
            // Increase spin rotation speed with more tricks
            this.state.spinRotation += 0.3 * this.state.trickCount;
            
            console.log('✅ Trick SUCCESS!', 'Chain:', this.state.trickChain);
            return true;
        }
        
        console.log('❌ Trick FAILED - still grounded!');
        return false;
    }
    
    // Check if player just bailed
    didBail() {
        const bailed = this.state.bailed;
        if (bailed) {
            this.state.bailed = false; // Reset after checking
        }
        return bailed;
    }
    
    // Check if landed clean
    landedCleanly() {
        return this.state.landedClean;
    }
    
    // Get current trick name
    getCurrentTrick() {
        return this.state.currentTrick;
    }
    
    // Get full trick chain for scoring
    getTrickChain() {
        return this.state.trickChain;
    }
    
    // Get trick count
    getTrickCount() {
        return this.state.trickCount;
    }
    
    // Clear trick chain (call after scoring)
    clearTrickChain() {
        this.state.trickChain = [];
        this.state.trickCount = 0;
        this.state.landedClean = false;
    }
    
    // Add downhill boost
    addDownhillBoost(amount = 0.02) {
        this.state.speed += amount;
    }
    
    // GRINDING
    checkGrinding(rails) {
        // Don't start grinding if already grounded
        if (this.state.grounded) {
            if (this.state.grinding) {
                this.state.grinding = false;
                this.state.currentRail = null;
            }
            return false;
        }
        
        // Only check when airborne and descending
        if (this.state.jumpVelocity > 0.05) return false;
        
        for (let rail of rails) {
            const railPos = rail.position;
            const dx = this.physicsCollider.position.x - railPos.x;
            const dz = this.physicsCollider.position.z - railPos.z;
            const horizontalDist = Math.sqrt(dx * dx + dz * dz);
            
            const dy = this.physicsCollider.position.y - railPos.y;
            
            // Collision detection
            if (horizontalDist < 3 && Math.abs(dy) < 2) {
                this.state.grinding = true;
                this.state.currentRail = rail;
                this.state.jumpVelocity = 0;
                this.physicsCollider.position.y = railPos.y + 1.0;
                return true;
            }
        }
        
        return false;
    }
    
    updateGrinding() {
        if (this.state.grinding && this.state.currentRail) {
            const railPos = this.state.currentRail.position;
            const dx = this.physicsCollider.position.x - railPos.x;
            const dz = this.physicsCollider.position.z - railPos.z;
            const horizontalDist = Math.sqrt(dx * dx + dz * dz);
            
            // Fall off if too far
            if (horizontalDist > 4) {
                this.state.grinding = false;
                this.state.currentRail = null;
                this.state.jumpVelocity = -0.1;
                return false;
            }
            return true;
        }
        return false;
    }
    
    jumpOffGrind() {
        if (this.state.grinding) {
            this.state.grinding = false;
            this.state.currentRail = null;
            this.state.jumpVelocity = 0.35;
            this.state.jumping = true;
            this.state.grounded = false;
            this.state.canTrick = true;
        }
    }
    
    // ==========================================
    // RAGDOLL SYSTEM
    // ==========================================
    
    enableRagdoll() {
        if (this.ragdollActive) return;
        this.ragdollActive = true;
        
        console.log('💀 RAGDOLL ENABLED');
        
        // Calculate current forward velocity for impulse direction
        const speed = this.state.speed;
        const rot = this.state.rotation;
        const forwardX = Math.sin(rot) * speed;
        const forwardZ = Math.cos(rot) * speed;
        const upVel = this.state.jumpVelocity || 0;
        
        // World position of the skater root
        const rootPos = this.skaterRoot.position.clone();
        const rootRotY = this.skaterRoot.rotation.y;
        
        // Hide the physics collider — ragdoll pieces replace it
        this.physicsCollider.isVisible = false;
        
        // Switch collider to static so it stops interacting
        this.physicsAggregate.body.setMotionType(BABYLON.PhysicsMotionType.STATIC);
        
        // Unparent each part, compute world position, give it physics
        for (const part of this.allParts) {
            // Compute world position from local position + root transform
            const localPos = part.position.clone();
            
            // Rotate local offset by root's Y rotation
            const cosR = Math.cos(rootRotY);
            const sinR = Math.sin(rootRotY);
            const worldX = rootPos.x + (localPos.x * cosR + localPos.z * sinR);
            const worldY = rootPos.y + localPos.y;
            const worldZ = rootPos.z + (-localPos.x * sinR + localPos.z * cosR);
            
            // Unparent and set world position
            part.parent = null;
            part.position = new BABYLON.Vector3(worldX, worldY, worldZ);
            part.rotation = new BABYLON.Vector3(0, rootRotY, 0);
            
            // Determine shape type based on mesh
            let shapeType = BABYLON.PhysicsShapeType.BOX;
            if (part.name === 'head') {
                shapeType = BABYLON.PhysicsShapeType.SPHERE;
            } else if (part.name.includes('wheel')) {
                shapeType = BABYLON.PhysicsShapeType.CYLINDER;
            }
            
            // Create dynamic physics body
            const aggregate = new BABYLON.PhysicsAggregate(
                part,
                shapeType,
                {
                    mass: part.name === 'body' ? 3 : (part.name === 'head' ? 2 : 1),
                    restitution: 0.4,
                    friction: 0.6
                },
                this.scene
            );
            
            // Apply velocity impulse — forward momentum + upward + random scatter
            const scatter = 3;
            const impulse = new BABYLON.Vector3(
                forwardX * 40 + (Math.random() - 0.5) * scatter,
                upVel * 30 + Math.random() * 4 + 2,
                forwardZ * 40 + (Math.random() - 0.5) * scatter
            );
            
            aggregate.body.applyImpulse(
                impulse,
                part.getAbsolutePosition()
            );
            
            // Add angular spin for tumble effect
            aggregate.body.setAngularVelocity(new BABYLON.Vector3(
                (Math.random() - 0.5) * 10,
                (Math.random() - 0.5) * 10,
                (Math.random() - 0.5) * 10
            ));
            
            this.ragdollAggregates.push(aggregate);
        }
        
        // Kill all movement state
        this.state.speed = 0;
        this.state.jumpVelocity = 0;
        this.state.jumping = false;
        this.state.spinning = false;
        this.state.grinding = false;
        this.state.stunned = false;
        this.state.stunnedTimer = 0;
    }
    
    disableRagdoll(spawnX = 0, spawnY = 5, spawnZ = -5) {
        if (!this.ragdollActive) return;
        
        console.log('✅ RAGDOLL RESET — Respawning');
        
        // Dispose all ragdoll physics bodies
        for (const agg of this.ragdollAggregates) {
            agg.dispose();
        }
        this.ragdollAggregates = [];
        
        // Re-parent all parts and restore original local positions
        for (const part of this.allParts) {
            part.parent = this.skaterRoot;
            part.position = this.partLocalPositions.get(part).clone();
            part.rotation = this.partLocalRotations.get(part).clone();
        }
        
        // Reset collider position and re-enable
        this.physicsCollider.position = new BABYLON.Vector3(spawnX, spawnY, spawnZ);
        this.physicsCollider.rotation = new BABYLON.Vector3(0, 0, 0);
        this.physicsAggregate.body.setMotionType(BABYLON.PhysicsMotionType.ANIMATED);
        this.physicsAggregate.body.disablePreStep = false;
        
        // Sync visual root
        this.skaterRoot.position = new BABYLON.Vector3(spawnX, spawnY - 1.0, spawnZ);
        this.skaterRoot.rotation = new BABYLON.Vector3(0, 0, 0);
        
        // Reset all state
        this.state.speed = 0;
        this.state.rotation = 0;
        this.state.jumpVelocity = 0;
        this.state.jumping = false;
        this.state.grounded = false;
        this.state.grinding = false;
        this.state.spinning = false;
        this.state.spinRotation = 0;
        this.state.currentTrick = '';
        this.state.landedClean = true;
        this.state.bailed = false;
        this.state.trickChain = [];
        this.state.trickCount = 0;
        this.state.stunned = false;
        this.state.stunnedTimer = 0;
        
        this.ragdollActive = false;
    }
    
    isRagdoll() {
        return this.ragdollActive;
    }
    
    // GETTERS
    getVelocity() {
        const forward = new BABYLON.Vector3(
            Math.sin(this.state.rotation),
            0,
            Math.cos(this.state.rotation)
        );
        return forward.scale(this.state.speed);
    }
    
    getSpeed() {
        return Math.abs(this.state.speed);
    }
    
    getPosition() {
        return this.physicsCollider ? this.physicsCollider.position : new BABYLON.Vector3(0, 0, 0);
    }
    
    isGrounded() {
        return this.state.grounded;
    }
    
    isGrinding() {
        return this.state.grinding;
    }
    
    isSpinning() {
        return this.state.spinning;
    }
    
    isStunned() {
        return this.state.stunned;
    }
    
    getRotation() {
        return this.state.rotation;
    }
    
    // VISUAL EFFECTS
    leanLeft() {
        if (this.deck) {
            this.deck.rotation.z = Math.min(this.deck.rotation.z + 0.05, 0.2);
        }
    }
    
    leanRight() {
        if (this.deck) {
            this.deck.rotation.z = Math.max(this.deck.rotation.z - 0.05, -0.2);
        }
    }
    
    resetLean() {
        if (this.deck) {
            this.deck.rotation.z *= 0.9;
        }
    }
    
    setDeckColor(color) {
        if (this.deck && this.deck.material) {
            this.deck.material.diffuseColor = color;
        }
    }
    
    setBodyColor(color) {
        if (this.body && this.body.material) {
            this.body.material.diffuseColor = color;
        }
    }
    
    remove() {
        if (this.skaterRoot) {
            this.skaterRoot.dispose();
        }
        if (this.physicsCollider) {
            this.physicsCollider.dispose();
        }
        if (this.physicsAggregate) {
            this.physicsAggregate.dispose();
        }
        if (this.rayHelper) {
            this.rayHelper.dispose();
        }
        
        console.log('🛹 Skater removed');
    }
}
