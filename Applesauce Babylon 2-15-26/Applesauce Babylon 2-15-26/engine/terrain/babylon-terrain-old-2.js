...
export class BabylonTerrain {
    ...
    spawnObject(config) {
        ...
        switch(config.type) {
            ...
            case 'building':
                ...
                break;
            case 'brickwall':
                // Brick walls handle their own physics internally
                this.createBrickWall(config);
                return; // Exit early - wall manages its own objects
            default:
     ...
     * Each brick is a separate physics object that can be broken/knocked down
     */
    createBrickWall(config) {
        ...
        // Brick material config
        const brickColor = config.brickColor || { r: 0.6, g: 0.2, b: 0.2 }; // Red-brown
        const brickMass = config.brickMass || 5;
        const restitution = config.restitution || 0.1; // Bounce
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < columns; x++) {
                ...
                // Offset every other row for stability/realism (brick pattern)
                const xOffset = (y % 2 === 0) ? 0 : brickWidth / 2;
                
                brick.position = new BABYLON.Vector3(
                    position.x + (x * brickWidth) + xOffset,
                    position.y + (y * brickHeight) + (brickHeight / 2), // Start half-brick height up
                    position.z
                );
                
                // Apply brick material
                ...
                // Add some variation to brick colors (slight randomness)
                ...
                // Add Havok physics - each brick is a separate dynamic object
                const aggregate = new BABYLON.PhysicsAggregate(
                    brick,
                    BABYLON.PhysicsShapeType.BOX,
                    { 
    ...
    getPhysicsShapeForType(type) {
        const shapeMap = {
            ...
            'brickwall': BABYLON.PhysicsShapeType.BOX // Individual bricks use BOX
        };
