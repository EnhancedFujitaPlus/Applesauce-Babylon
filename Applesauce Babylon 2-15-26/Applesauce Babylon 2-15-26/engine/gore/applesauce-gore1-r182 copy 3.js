/**
 * APPLESAUCE Gore Module for Three.js r182
...
export class ApplesauceGore {
    ...
    // BLOOD SPLATTER
    // ===================================
    createBloodSplatter(position, velocity, amount = 3) {
    ...
    createBloodParticle(position, baseVelocity) {
        ...
        particle.velocity = new THREE.Vector3(
            baseVelocity.x + (Math.random() - 0.5) * 0.1,
            Math.random() * 0.4 + 0.2,
            baseVelocity.z + (Math.random() - 0.5) * 0.1
        );
        
        particle.lifetime = 100000; // 1000 frames persistence
    ...
    createBloodPool(position, size = 2) {
        ...
        pool.lifetime = 100000; // Lasts longer than particles
    ...
    // GIBS (BODY PARTS)
    // ===================================
    createGibs(position, velocity, count = 50) {
    ...
    createMassiveSplatter(position, velocity) {
        ...
        this.createBloodSplatter(position, velocity, 500);
        this.createBloodPool(position, 3);
        this.createGibs(position, velocity, 16);
    }
