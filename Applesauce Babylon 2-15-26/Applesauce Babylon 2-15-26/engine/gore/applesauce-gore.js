...
export class ApplesauceGore {
    ...
    cameraShake(intensity = 0.3) {
        ...
        const shakeFrames = 15;
        let frame = 0;
        
        const shakeInterval = setInterval(() => {
            if (frame >= shakeFrames) {
                clearInterval(shakeInterval);
                this.engine.camera.position.set(originalPos.x, originalPos.y, originalPos.z);
                return;
            }
            
            const decay = 1 - (frame / shakeFrames);
            this.engine.camera.position.x = originalPos.x + (Math.random() - 0.5) * intensity * decay;
            this.engine.camera.position.y = originalPos.y + (Math.random() - 0.5) * intensity * decay;
            this.engine.camera.position.z = originalPos.z + (Math.random() - 0.5) * intensity * decay;
    ...
    // ===================================
    createArterialSpray(position, direction, intensity = 1) {
    ...
    createMassiveSplatter(position, velocity) {
        ...
        this.cameraShake(0.3 * comboMultiplier);
