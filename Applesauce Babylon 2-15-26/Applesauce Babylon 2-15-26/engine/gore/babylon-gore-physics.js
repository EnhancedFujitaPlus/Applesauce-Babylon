...
export class BabylonGorePhysics {
    ...
    /**
     * Spawn blood using a real Babylon ParticleSystem (with fallback)
     */
    spawnBlood(position, velocity, severity) {
        const particleCount = severity === 'CATASTROPHIC' ? 500 :
                             severity === 'SEVERE' ? 300 : 150;
        ...
        try {
            const emitter = new BABYLON.TransformNode("bloodEmitter", this.scene);
            ...
            //blood texture
            ps.particleTexture = new BABYLON.Texture("https://assets.babylonjs.com/textures/blood.png", this.scene);

            ps.emitter = emitter;
            
            ps.color1 = new BABYLON.Color4(0.8, 0, 0, 1);
            ps.color2 = new BABYLON.Color4(0.5, 0, 0, 1);
            ...
            ps.minLifeTime = 0.3;
            ps.maxLifeTime = 1.5;
            ps.emitRate = particleCount * 10;
            ps.gravity = new BABYLON.Vector3(0, -9.8, 0);
            ps.minEmitPower = velocity * 0.3;
            ps.maxEmitPower = velocity * 0.8;
            ps.addSizeGradient(0, 0.05, 0.1); // Start small
            ps.addSizeGradient(1.0, 0.2, 0.3); // Grow as it "splats"
